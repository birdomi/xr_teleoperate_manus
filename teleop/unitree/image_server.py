...
def get_index_from_by_id(id_pattern):
    # /dev/v4l/by-id/ 폴더에서 패턴에 맞는 파일 찾기
    files = glob.glob(f"/dev/v4l/by-id/*{id_pattern}*index0") # index0이 보통 스트림용

    if files:
        # 심볼릭 링크가 가리키는 실제 경로 확인 (예: ../../video6)
        real_path = os.path.realpath(files[0])
        # 숫자 추출
        dev_name = os.path.basename(real_path) # video6
                    ...
                    for cam in self.wrist_cameras:
                        if self.wrist_camera_type == 'opencv':
                            ...
                            if color_image is None:
                                logger_mp.error("[Image Server] Wrist camera frame read is error.")
                                break
                        if cam.serial_number == '218622272395':
                            color_image = cv2.flip(color_image, -1)
...
if __name__ == "__main__":
    ...
    idx = get_index_from_by_id("3D_USB_Camera")
    ...
    #    'head_camera_type': 'realsense',
       # 'head_camera_type': 'zed',
        ...
        'head_camera_id_numbers': [idx],
        'wrist_camera_type': 'realsense',
        'wrist_camera_image_shape': [480, 640],  # Wrist camera resolution
        'wrist_camera_id_numbers': ["218722270116", "218622272395"],   
    }
