    ...
    def send_process(self):
        try:
            while True:
                ...
                if head_color.shape[0] < 480 and self.wrist_cameras:
                    pad_bottom = 480 - head_color.shape[0]
              #      print(pad_bottom)
                    head_color = cv2.copyMakeBorder(
                        head_color,
                        0, pad_bottom,   # top=0, bottom=pad_bottom
                        0, 0,            # left=0, right=0
                        cv2.BORDER_CONSTANT,
                        value=(0, 0, 0)  # 검정색 패딩
                    )
              #  print(head_color.shape)
