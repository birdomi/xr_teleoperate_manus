...
class Inspire_Controller:
    def __init__(self, left_hand_array, right_hand_array, dual_hand_data_lock=None, dual_hand_state_array=None,
                 dual_hand_action_array=None, fps=100.0, Unit_Test=False, network_interface="enp110s0"): # Added network_interface
        ...
        while not (any(self.left_hand_state_array.get_obj()) or any(self.right_hand_state_array.get_obj())):
            ...
            time.sleep(0.01)
            wait_count +=1
            if wait_count > 5000: # Timeout after 5 seconds
    ...
    def replay(self, left_hand, right_hand):
        running = True

        scaled_left_cmd = [int(np.clip(val * 1000, 0, 1000)) for val in left_hand]
        scaled_right_cmd = [int(np.clip(val * 1000, 0, 1000)) for val in right_hand]        
