...
from televuer import TeleVuerWrapperTouch
...
if __name__ == '__main__':
    ...
    # TeleVuer (XR) wrapper
    # =========================
    tv_wrapper = TeleVuerWrapperTouch(
        binocular=BINOCULAR,
    ...
    try:
        ...
        while running:
            ...
            # tactile → VR overlay 업데이트
            if args.ee == "brainco":
                with left_hand_tactile_array.get_lock():
                    tv_wrapper.update_tactile(
                        np.array(left_hand_tactile_array[:]),
                        np.array(right_hand_tactile_array[:]),
                    )
