...
parser.add_argument("--enable_robot_camera", action="store_true", default=False, help="enable robot camera sensor and OpenCV preview window")
parser.add_argument("--robot_camera_name", type=str, default="robot_camera", help="scene sensor name used for the robot camera")
...
if args_cli.enable_robot_camera:
...
def _append_csv_name(value: str, name: str) -> str:
    names = _csv_names(value)
    if name and name not in names:
        names.append(name)
    return ",".join(names)


def _remove_csv_name(value: str, name: str) -> str:
    return ",".join(item for item in _csv_names(value) if item != name)


def _configure_robot_camera(env_cfg) -> None:
    """Enable a robot-mounted camera in the scene config when requested."""
    if not args_cli.enable_robot_camera:
        return

    camera_name = args_cli.robot_camera_name
    try:
        scene_cfg = env_cfg.scene
        if hasattr(scene_cfg, camera_name):
            camera_cfg = getattr(scene_cfg, camera_name)
            print(f"[camera] using existing scene camera '{camera_name}' ({getattr(camera_cfg, 'prim_path', 'unknown prim')})")
        else:
            from tasks.common_config import CameraPresets

            if args_cli.robot_type == "h1_2":
                camera_cfg = CameraPresets.h12_world_camera()
            else:
                camera_cfg = CameraPresets.g1_world_camera()
            setattr(scene_cfg, camera_name, camera_cfg)
            print(f"[camera] added scene camera '{camera_name}' ({camera_cfg.prim_path})")

        args_cli.camera_include = _append_csv_name(args_cli.camera_include, camera_name)
        args_cli.camera_exclude = _remove_csv_name(args_cli.camera_exclude, camera_name)
        args_cli.enable_robot_camera = True
        print(f"[camera] camera include list: {args_cli.camera_include}")
    except Exception as e:
        print(f"[camera] failed to configure robot camera: {e}")


def _camera_numpy_rgb(env, camera_name: str):
    try:
        ...
        frame = sensor.data.output.get("rgb")
        if frame is None:
            return None
        image = frame[0]
        ...
        print(f"[camera] failed to read '{camera_name}': {e}")
        return None


_robot_camera_preview_ready = False


def _show_robot_camera_preview(env, camera_name: str) -> None:
    global _robot_camera_preview_ready
    image = _camera_numpy_rgb(env, camera_name)
    if image is None:
        return
    try:
        import cv2

        if not _robot_camera_preview_ready:
            cv2.namedWindow("Robot Camera", cv2.WINDOW_NORMAL)
            cv2.resizeWindow("Robot Camera", image.shape[1], image.shape[0])
            _robot_camera_preview_ready = True
        cv2.imshow("Robot Camera", cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
        cv2.waitKey(1)
    except Exception as e:
        print(f"[camera] failed to show robot camera preview: {e}")


def _close_robot_camera_preview() -> None:
    global _robot_camera_preview_ready
    if not _robot_camera_preview_ready:
        return
    try:
        ...
        cv2.destroyWindow("Robot Camera")
    except Exception:
        pass
    _robot_camera_preview_ready = False


_camera_base_quat_cache = {}
_camera_target_last_marker = None
...
def _quat_multiply_wxyz(lhs, rhs):
    lw, lx, ly, lz = lhs
    rw, rx, ry, rz = rhs
...
def _target_to_quat_wxyz(target):
    if not target:
        return None
    if "quat_wxyz" in target:
        return _normalize_quat_wxyz(target["quat_wxyz"])
    if "rpy" in target:
        return _rpy_to_quat_wxyz(target["rpy"])
    return None


def _resolve_robot_camera_prim_path(env, camera_name: str) -> str | None:
    try:
        if camera_name not in env.scene.keys():
            return None
        sensor = env.scene[camera_name]
        cfg = getattr(sensor, "cfg", None)
        prim_path = getattr(cfg, "prim_path", None) or getattr(sensor, "prim_path", None)
        if not prim_path:
            return None

        candidates = [
            str(prim_path),
            str(prim_path).replace("env_.*", "env_0"),
            str(prim_path).replace("env_.*/", "env_0/"),
        ]
        ...
        print(f"[camera] failed to resolve camera prim path: {e}")
        return None


def _get_prim_orient_wxyz(prim):
    attr = prim.GetAttribute("xformOp:orient")
    if attr and attr.IsValid():
        value = attr.Get()
        if value is not None:
            imag = value.GetImaginary()
            return _normalize_quat_wxyz([value.GetReal(), imag[0], imag[1], imag[2]])
    return [1.0, 0.0, 0.0, 0.0]


def _set_prim_orient_wxyz(prim, quat):
    ...
    attr_type = str(orient_op.GetAttr().GetTypeName())
    if attr_type == "quatf":
        value = Gf.Quatf(quat[0], Gf.Vec3f(quat[1], quat[2], quat[3]))
    else:
        value = Gf.Quatd(quat[0], Gf.Vec3d(quat[1], quat[2], quat[3]))
    orient_op.Set(value)


def _apply_robot_camera_target(env, camera_target_dds, camera_name: str) -> None:
    global _camera_target_last_marker
    if camera_target_dds is None:
        return

    try:
        target = camera_target_dds.get_camera_target()
        if not target:
            return

        marker = (target.get("seq"), target.get("timestamp"))
        if marker == _camera_target_last_marker:
            return

        target_quat = _target_to_quat_wxyz(target)
        if target_quat is None:
            return

        prim_path = _resolve_robot_camera_prim_path(env, camera_name)
        if not prim_path:
            return
        ...
        base_quat = _camera_base_quat_cache.get(prim_path)
        if base_quat is None:
            base_quat = _get_prim_orient_wxyz(prim)
            _camera_base_quat_cache[prim_path] = base_quat

        mode = str(target.get("mode", "absolute")).lower()
        if mode == "raw":
            desired_quat = target_quat
        elif mode == "relative":
            desired_quat = _quat_multiply_wxyz(target_quat, _get_prim_orient_wxyz(prim))
        else:
            desired_quat = _quat_multiply_wxyz(target_quat, base_quat)

        smoothing = float(target.get("smoothing", 0.0) or 0.0)
        if smoothing > 0.0:
            current_quat = _get_prim_orient_wxyz(prim)
            smoothing = max(0.0, min(0.99, smoothing))
            desired_quat = _normalize_quat_wxyz([
                current_quat[i] * smoothing + desired_quat[i] * (1.0 - smoothing)
                for i in range(4)
            ])

        _set_prim_orient_wxyz(prim, desired_quat)
        _camera_target_last_marker = marker
    except Exception as e:
        print(f"[camera] failed to apply robot camera target: {e}")
...
def main():
    ...
    try:
        ...
        _configure_robot_camera(env_cfg)
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            if args_cli.enable_robot_camera:
                try:
                    cam_state.set_robot_camera_name(args_cli.robot_camera_name)
                except Exception:
                    pass
            try:
    ...
    env.sim.reset()
    env.reset()
    if args_cli.enable_robot_camera and (getattr(args_cli, "headless", False) or args_cli.no_render):
        print("[camera] robot camera sensor enabled; OpenCV preview is disabled because GUI rendering is disabled")
    ...
    if not args_cli.replay_data:
        ...
        try:
            ...
            return
        if args_cli.enable_robot_camera:
            camera_target_dds = dds_manager.get_object("robot_camera_target")
    ...
    try:
        ...
        with contextlib.suppress(KeyboardInterrupt), torch.inference_mode():
            while simulation_app.is_running() and controller.is_running:
                ...
                _apply_robot_camera_target(env, camera_target_dds, args_cli.robot_camera_name)

                # execute control step (in main thread, support rendering)
                controller.step()
                if args_cli.enable_robot_camera and not getattr(args_cli, "headless", False) and not args_cli.no_render:
                    _show_robot_camera_preview(env, args_cli.robot_camera_name)
